...
def parse_interactioninfo_entry(data, pos):
    """Auto-generated parser for interactioninfo entries."""
    entry = {}
    ...
    entry['field_10'] = struct.unpack_from('<f', data, pos)[0]; pos += 4
    entry['field_11'] = data[pos]; pos += 1
    entry['field_12'] = data[pos]; pos += 1
    entry['field_13'] = struct.unpack_from('<f', data, pos)[0]; pos += 4
    _slen = struct.unpack_from('<I', data, pos)[0]; pos += 4
    entry['field_14'] = data[pos:pos + _slen].decode('utf-8', errors='replace'); pos += _slen
    ...
    entry['field_55'] = struct.unpack_from('<f', data, pos)[0]; pos += 4
